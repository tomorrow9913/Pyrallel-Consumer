...
## 2. Stable Public Contract 기본값 (1.0.0 Gate)

아래 기본값은 1.0.0 릴리스 게이트 public contract로 잠금된 값입니다
(기준: `docs/blueprint/04-open-decisions.md`, GitHub `#34`).

- **정렬 모드 기본값**: `key_hash`
- **DLQ payload 기본값**: `full` (production 권장값: `metadata_only`)
- **커밋 의미론**: stable public guarantee는 `on_complete`만 포함
- **리밸런스/재시작 상태 전략 기본값**: `contiguous_only`
  (`metadata_snapshot`은 opt-in + fail-closed)
