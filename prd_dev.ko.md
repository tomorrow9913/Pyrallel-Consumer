# Pyrallel Consumer: 개발 명세서 (`prd_dev.ko.md`)

이 문서는 Python `asyncio` 환경에 최적화된 고성능 Kafka 병렬 처리 라이브러리 `Pyrallel-Consumer`의 보존된 한국어 개발 명세 원문입니다. 영문 canonical entry는 [`prd_dev.md`](./prd_dev.md)이며, repository-wide language policy는 [`docs/internal-doc-language-policy.md`](./docs/internal-doc-language-policy.md)에 정리되어 있습니다.
