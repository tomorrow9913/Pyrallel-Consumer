# Pyrallel Consumer – 최종 설계 해설서 (`prd.ko.md`)

이 문서는 `Pyrallel-Consumer`의 보존된 한국어 설계 해설 원문입니다. 영문 canonical entry는 [`prd.md`](./prd.md)이며, repository-wide language policy는 [`docs/internal-doc-language-policy.md`](./docs/internal-doc-language-policy.md)에 정리되어 있습니다.
...
class ExecutionEngine(ABC):
    ...
    @abstractmethod
    async def poll_completed_events(self, batch_limit: int = 1000) -> list[CompletionEvent]: ...
    @abstractmethod
...
### 4.4. `wait_for_completion()`의 역할
- **원칙**: `wait_for_completion()`은 completion-driven rescheduling을 위해 사용되는 보조 대기 API입니다.
- **이유**: BrokerPoller는 Kafka consume cadence만 기다리지 않고, 새 완료 이벤트가 도착하면 즉시 completion drain → `schedule()` 경로를 다시 태워 ordered backlog의 head-of-line blocking을 줄일 수 있어야 합니다.
- **주의**: completion monitor는 선택적이며(`strict_completion_monitor_enabled`), stale completion은 epoch fence를 통과하지 못하면 commit state를 전진시키지 않아야 합니다.
...
- **추가 큐 한도**: 내부 virtual queue 총 길이가 `queue_max_messages`를 초과하면 pause되고, queued count가 `queue_max_messages * 0.7` 이하로 내려가야 resume 후보가 됩니다.
