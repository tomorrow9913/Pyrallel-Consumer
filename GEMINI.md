...
## 최근 업데이트 (2026-04-25)
- PR #124 review fix - pending worker-pipe retry boundary (2026-04-30 KST): Codex review가 지적한 pending worker-pipe dispatch recovery의 retry cap 불일치를 수정했습니다. `ProcessRegistrySupport.recover_dead_worker_items()`는 `requeue_attempts >= max_retries`에서 failure completion을 내는데, `ProcessExecutionEngine._filter_recoverable_pending_pipe_dispatches()`는 `>` 조건이라 동일 retry boundary에서 pending pipe dispatch가 한 번 더 requeue될 수 있었습니다. 직접 경계 회귀 테스트 `test_pending_worker_pipe_filter_caps_retry_boundary`를 추가한 뒤 조건을 `>=`로 맞췄고, focused verification은 `UV_CACHE_DIR=.uv-cache uv run pytest tests/unit/execution_plane/test_process_execution_engine.py::test_pending_worker_pipe_filter_caps_retry_boundary tests/unit/execution_plane/test_process_execution_engine.py::test_ensure_workers_alive_caps_pending_worker_pipe_retries -q` -> `2 passed`입니다.
