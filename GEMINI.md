...
### 5.49 develop -> main conflict-resolution prep (2026-04-24)

- Created isolated worktree `.worktrees/codex-develop-main-sync` from `origin/develop`, merged `origin/main` there, and resolved the current add/add/content conflicts by keeping the newer `develop` variants for Dependabot config, benchmark workflow pins, observability metrics design docs, release-gate/supply-chain regression assets, and process-engine unit coverage. This preserves the in-progress process-throughput worktree while surfacing the backmerge state needed for PR #113 follow-up.
- Verification: `UV_CACHE_DIR=.uv-cache uv run pytest tests/unit/execution_plane/test_process_execution_engine.py tests/unit/benchmarks/test_release_gate.py tests/unit/test_supply_chain_assets.py tests/unit/metrics/test_monitoring_assets.py -q` -> 45 passed after restoring the `develop` `uv.lock` to avoid an invalid merged lockfile shape.
