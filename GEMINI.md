...
## 최근 업데이트 (2026-04-25)
- PR #124 monitoring artifact-name follow-up (2026-04-30 KST): `ci_monitoring` workflow의 benchmark artifact name이 pull_request 이벤트에서 `github.ref_name`(`124/merge`)를 포함해 `actions/upload-artifact@v7` 금지 문자인 `/` 때문에 실패하는 문제를 수정했습니다. `PYRALLEL_BENCHMARK_ARTIFACT_NAME`은 PR 이벤트에서는 `github.event.pull_request.number`, push/manual 이벤트에서는 `github.ref_name`을 사용하도록 변경했고, `tests/unit/benchmarks/test_benchmark_runtime.py::test_ci_monitoring_workflow_uploads_benchmark_artifact_with_bound_name`에 슬래시 포함 ref_name template을 쓰지 않는 회귀 가드를 추가했습니다.
- PR #124 review fix - pending worker-pipe retry boundary (2026-04-30 KST): Codex review가 지적한 pending worker-pipe dispatch recovery의 retry cap 불일치를 수정했습니다. `ProcessRegistrySupport.recover_dead_worker_items()`는 현재 `requeue_attempts >= max_retries`에서 failure completion을 내고, retry 가능한 payload만 `requeue_attempts + 1`로 복구합니다. pending worker-pipe 경로도 같은 check-then-increment 구조가 되도록 `WorkerPipesProcessTransport.recover_pending_dispatches()`의 사전 increment를 제거하고, `ProcessExecutionEngine._filter_recoverable_pending_pipe_dispatches()`가 retry boundary 확인 후 increment하도록 맞췄습니다. 직접 경계 회귀 테스트 `test_pending_worker_pipe_filter_caps_retry_boundary`와 최종 retry 허용 테스트 `test_pending_worker_pipe_filter_allows_final_retry`를 추가했습니다. Focused verification은 `UV_CACHE_DIR=.uv-cache uv run pytest tests/unit/execution_plane/test_process_execution_engine.py::test_worker_pipe_recover_pending_dispatches_returns_identity_metadata tests/unit/execution_plane/test_process_execution_engine.py::test_ensure_workers_alive_requeues_pending_worker_pipe_dispatch tests/unit/execution_plane/test_process_execution_engine.py::test_pending_worker_pipe_filter_caps_retry_boundary tests/unit/execution_plane/test_process_execution_engine.py::test_pending_worker_pipe_filter_allows_final_retry tests/unit/execution_plane/test_process_execution_engine.py::test_ensure_workers_alive_caps_pending_worker_pipe_retries -q` -> `5 passed`입니다.
- Worker-pipe transport slot/pending ownership refactor 진행 (2026-04-25 KST): `ProcessExecutionEngine`가 더 이상 worker-pipe pending dispatch dict나 queue-slot semaphore를 직접 소유하지 않고, 해당 상태를 `WorkerPipesProcessTransport` 내부로 밀어 넣었습니다. 엔진은 transport seam(`handle_registry_event`, `recover_pending_dispatches`, `clear_pending_dispatches`)만 통해 worker-pipe dispatch recovery를 다루도록 좁혔고, 관련 단위 테스트도 transport-owned pending/slot 상태를 기준으로 갱신했습니다. 검증은 `./.venv/bin/python -m pytest tests/unit/execution_plane/test_process_execution_engine.py -q` -> `46 passed`, `./.venv/bin/python -m pytest tests/unit/execution_plane/test_execution_engine_contract.py -q` -> `2 passed`, `./.venv/bin/ruff check pyrallel_consumer/execution_plane/process_engine.py pyrallel_consumer/execution_plane/process_transport.py pyrallel_consumer/execution_plane/process_transport_shared_queue.py pyrallel_consumer/execution_plane/process_transport_worker_pipes.py tests/unit/execution_plane/test_process_execution_engine.py` -> clean, `./.venv/bin/python -m mypy pyrallel_consumer/execution_plane/process_engine.py pyrallel_consumer/execution_plane/process_transport.py pyrallel_consumer/execution_plane/process_transport_shared_queue.py pyrallel_consumer/execution_plane/process_transport_worker_pipes.py` -> success였습니다.
- Release-gate metadata/provenance binding end-to-end validation (2026-04-25 KST): worker-1이 추가한 benchmark JSON `artifact_metadata` emission(`github_repository`, `git_ref`, `git_commit_sha`)에 맞춰 `benchmarks/release_gate.py` provenance binder를 확장해 새로운 metadata schema를 우선 읽고, legacy `artifact_provenance`도 fallback으로 허용하도록 했습니다. `tests/unit/benchmarks/test_release_gate.py`는 이제 artifact_metadata 기반 PASS/mismatch/missing binding과 legacy fallback을 검증합니다. 검증은 `./.venv/bin/python -m pytest tests/unit/benchmarks/test_release_gate.py tests/unit/benchmarks/test_benchmark_runtime.py::test_build_artifact_metadata_prefers_github_environment tests/unit/benchmarks/test_benchmark_runtime.py::test_run_benchmark_writes_artifact_metadata tests/unit/benchmarks/test_stats.py::test_write_results_json_preserves_artifact_metadata -q` -> `24 passed`, `./.venv/bin/ruff check benchmarks/release_gate.py benchmarks/run_parallel_benchmark.py benchmarks/stats.py tests/unit/benchmarks/test_release_gate.py tests/unit/benchmarks/test_benchmark_runtime.py tests/unit/benchmarks/test_stats.py` -> clean, `./.venv/bin/python -m mypy benchmarks/release_gate.py benchmarks/run_parallel_benchmark.py benchmarks/stats.py` -> success였습니다.
- Release-gate artifact provenance emission/workflow plumbing (2026-04-25 KST): `benchmarks/run_parallel_benchmark.py`가 이제 top-level `artifact_metadata`를 JSON summary에 함께 기록하며, GitHub Actions env(`GITHUB_SHA`, `GITHUB_REF*`, `GITHUB_RUN_*`, `GITHUB_WORKFLOW*`)를 우선 사용하고 로컬 실행에서는 git fallback으로 commit/ref를 채웁니다. `benchmarks/stats.py`는 `artifact_metadata` payload를 보존하고, `ci_monitoring.yml`은 `PYRALLEL_BENCHMARK_ARTIFACT_NAME` 기반 benchmark JSON artifact 업로드를 추가해 workflow-side provenance 이름도 남기도록 정리했습니다. 회귀 가드로 `tests/unit/benchmarks/test_stats.py`와 `tests/unit/benchmarks/test_benchmark_runtime.py`에 artifact metadata serialization, env binding, run_benchmark write-through, workflow upload asset 테스트를 추가했고, RED→GREEN 검증은 `./.venv/bin/python -m pytest tests/unit/benchmarks/test_stats.py tests/unit/benchmarks/test_benchmark_runtime.py -q` -> `47 passed`였습니다.
- Release-gate artifact provenance binding follow-up (2026-04-25 KST): `benchmarks/release_gate.py`가 이제 각 benchmark JSON summary의 top-level `artifact_provenance.repository/git_ref/git_sha` binding을 필수로 요구하고, 반복 artifact들이 동일한 repository/ref/sha에 묶여 있는지 검증합니다. `tests/unit/benchmarks/test_release_gate.py`에는 provenance 누락, binding mismatch, summary surface 회귀 테스트를 추가했습니다. 검증은 `./.venv/bin/python -m pytest tests/unit/benchmarks/test_release_gate.py -q` -> `20 passed`였습니다.

## 최근 업데이트 (2026-04-24)
- Worker-pipe dispatch payload guard (2026-04-25 KST): `WorkerPipesProcessTransport`가 이제 parent-side에서 직렬화된 pipe payload를 전송 전에 검증해 `msgpack_max_bytes`를 초과하는 oversized payload는 `ValueError("payload_too_large")`, 비정상 msgpack bytes는 `ValueError("invalid_worker_pipe_payload")`로 즉시 거절합니다. 덕분에 worker가 빈 completion/failure envelope를 내보내며 pending dispatch를 orphan시키는 경로를 submit 시점에 차단했고, `tests/unit/execution_plane/test_process_execution_engine.py`에 oversized/invalid payload 회귀 테스트를 추가했습니다. 검증은 `./.venv/bin/python -m pytest tests/unit/execution_plane/test_process_execution_engine.py -q` -> `43 passed`, `./.venv/bin/ruff check pyrallel_consumer/execution_plane/process_engine.py pyrallel_consumer/execution_plane/process_transport_worker_pipes.py tests/unit/execution_plane/test_process_execution_engine.py` -> clean, `./.venv/bin/python -m mypy pyrallel_consumer/execution_plane/process_engine.py pyrallel_consumer/execution_plane/process_transport_worker_pipes.py` -> success였습니다.
- BrokerPoller metrics compatibility projection follow-up (2026-04-25 KST): `BrokerPoller.get_metrics()`가 더 이상 legacy `ProcessBatchMetrics` instance만 그대로 전달하지 않고, `BrokerRuntimeSupport`의 projection helper를 재사용해 `EngineRuntimeDiagnostics(process=...)` envelope도 `SystemMetrics.process_batch_metrics` v1 compatibility field로 투영하도록 맞췄습니다. 회귀 가드로 `tests/unit/control_plane/test_broker_poller_metrics.py`에 envelope projection 테스트를 추가했습니다. 검증은 `UV_CACHE_DIR=/tmp/uv-cache uv run pytest tests/unit/control_plane/test_broker_poller_metrics.py -q` -> `11 passed`였습니다.
...
### 5.53 Engine boundary docs contract wording (2026-04-25)

- TDD(red): extended `tests/unit/test_public_contract_v1.py` and `tests/unit/test_operations_evidence_assets.py` to require docs to say commit clamping comes from the control-plane `WorkManager` ledger and that `process_batch_metrics` remains a v1 compatibility projection. Focused verification failed as expected before doc updates (`UV_CACHE_DIR=.uv-cache uv run pytest tests/unit/test_public_contract_v1.py tests/unit/test_operations_evidence_assets.py -q` -> 2 failed, 8 passed).
- Green: updated `docs/operations/public-contract-v1.md`, `docs/operations/guide.en.md`, `docs/operations/guide.ko.md`, and `README.md` so the public contract and operator guidance now describe the control-plane commit-clamp boundary, keep engine-private registries in recovery/diagnostics scope, and preserve `process_batch_metrics` as the v1 compatibility projection while generic engine diagnostics remain internal/additive.

### 5.54 Benchmark mypy cleanup follow-up (2026-04-25)

- Worker-1 green: cleared the remaining benchmark-only mypy failures without changing benchmark behavior by tightening `benchmarks/baseline_consumer.py` Kafka error handling (single local error object, public constant lookups via `getattr`, safe EOF offset formatting), normalizing optional `Input.placeholder` values and `.field-error` widget iteration in `benchmarks/tui/app.py`, and marking optional `yappi` imports as untyped in the benchmark profiler entrypoints.
- Verification: `PATH="$PWD/.venv/bin:$PATH" mypy benchmarks` -> success; `PATH="$PWD/.venv/bin:$PATH" ruff check benchmarks/baseline_consumer.py benchmarks/tui/app.py benchmarks/profile_benchmark_yappi.py benchmarks/run_parallel_benchmark.py` -> pass; `PATH="$PWD/.venv/bin:$PATH" pytest tests/unit/benchmarks/test_benchmark_runtime.py tests/unit/benchmarks/test_tui_app.py tests/unit/benchmarks/test_tui_results_report.py` -> 79 passed; `PATH="$PWD/.venv/bin:$PATH" python -m benchmarks.run_parallel_benchmark --help` -> pass; `PATH="$PWD/.venv/bin:$PATH" python -m benchmarks.profile_benchmark_yappi --help` -> pass.
