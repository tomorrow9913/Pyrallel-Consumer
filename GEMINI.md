...
- GH-34 stable public contract lock 단계 (2026-04-17): `docs/blueprint/04-open-decisions.md`를 1.0.0 gate 기준 lock 문서로 전환해 ordering 기본값(`key_hash`), DLQ payload 기본값(`full` + production `metadata_only` 가이드), commit public surface(`on_complete` only), rebalance/restart 기본 전략(`contiguous_only`, `metadata_snapshot` opt-in fail-closed)을 명시적으로 고정했습니다. `docs/operations/release-readiness.md`의 P0 `핵심 public contract 동결` 항목을 완료 처리하고 증거 라인을 갱신했으며, `README.md`/`README.ko.md`와 `docs/operations/guide.en.md`/`docs/operations/guide.ko.md`에 동일 계약 결정을 동기화했습니다. 이번 단계는 문서 잠금 작업이라 별도 테스트 실행은 하지 않았습니다.
