...
def test_results_modal_renders_ordering_grouped_winner_sections(tmp_path: Path) -> None:
    ...
    assert "ORDER: key_hash" in key_hash_text
    assert "sleep 1등" in key_hash_text
    assert "async" in key_hash_text
    assert "cpu 1등" in key_hash_text
    assert "process" in key_hash_text
    assert "ORDER: partition" in partition_text
    assert "io 1등" in partition_text
...
def test_results_modal_hides_unselected_workload_winner_cards(tmp_path: Path) -> None:
    ...
    assert "cpu 1등" not in modal._winner_section_text("key_hash")
...
async def test_run_screen_opens_results_modal_after_completion(
    ...
    async with app.run_test() as pilot:
        ...
        key_hash_winners = summary_screen.query_one("#results-order-key_hash", Static)
    ...
    assert "Overview:" in str(overview_row.content)
